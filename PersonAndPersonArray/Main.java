public class Main {
        public static void main(String[] args) {
                Person adam = new Person.Builder().firstName("Adam").lastName("Abacki").age(32).weight(78).height(186)
                                .build();

                Person karol = new Person.Builder().firstName("Karol").lastName("Babacki").age(21).weight(74)
                                .height(183).build();

                Person piotr = new Person.Builder().firstName("Piotr").lastName("Cabacki").age(25).weight(92)
                                .height(191).build();

                Person none = new Person.Builder().build();

                // Person person = new Person(); // should not work
                // Person person = new Person("Adam", "Kowalski", 20, 76, 176); // should not
                // work
